...
var DEFAULT_HOSTNAME = new remoteip_1.RemoteIp().getIp();
var debug = false;
var times = 10;
// gloabl var
var waitDone = false;
var che = { hostname: DEFAULT_HOSTNAME, server: 'tmp' };
var dockerContent;
// requirements
...
function up() {
    ...
    cheBoot();
    dockerContent = new recipebuilder_1.RecipeBuilder().getDockerContent();
...
function initCheFolders() {
    // create .che folder
    try {
        fs.mkdirSync(dotCheFolder, 484);
    }
    catch (e) {
    }
    // create .che/workspaces folder
    try {
        fs.mkdirSync(workspacesFolder, 484);
    }
    catch (e) {
    }
    // create .che/conf folder
    try {
        fs.mkdirSync(confFolder, 484);
    }
    catch (e) {
    }
